...
class Autoencoder(nn.Module):
    ...
    def train_autoencoder(self, dataloader, criterion, optimizer=torch.optim.Adam,
                          epochs: int = 50,
        ...
        for epoch in range(epochs):
            ...
            n_datapoints = dataloader.dataset.data.shape[0]
            if writer is None:
                print("\tEpoch", epoch + 1, "\AvgLoss: ", np.round(overall_loss / n_datapoints, 5))
            else: 
                writer.add_scalar('Loss/Reconstruction', overall_loss/n_datapoints, epoch)
                writer.flush()

    def greedy_pretrain(self, dataloader, criterion, optimizer=torch.optim.Adam,
                        epochs: int = 50,
                        device=torch.device('cpu'),
                        optimizer_params: dict = {'lr': 1e-3, 'betas': (0.9, 0.999)},
                        loss_params: dict = {'p': 2, 'pow': 2}, 
