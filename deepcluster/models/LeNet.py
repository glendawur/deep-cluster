...
class DeepSpectralCluster(nn.Module):
    ...
    def predict(self, x, alpha: float = None):
        ...
        labels = F.softmax(-alpha*torch.cdist(x, self.centers), dim = 1)
        return labels


def train_net(model, dataloader_train, dataloader_test =  None, graph_k: int = 16, epochs: int = 200,
              loss_w: dict = {'attr': 0.5, 'rep': 0.5, 'penalty': 0.5, 'cls': 0.5}):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x, y) in enumerate(dataloader_train):            
            ...
            print(torch.sort(dist, dim=1)[0].shape)
            ...
            labels = torch.nn.functional.softmax(-alpha*torch.cdist(z, model.centers), dim = 1)
        ...
        if dataloader_test is not None:
            ...
            z = model(x).detach()
            labels = model.predict(z, alpha = 2000).argmax(dim = 1)
            writer.add_scalar('ARI on validation', adjusted_rand_score(labels, sample_y), epoch)
        ...
        alpha = (2 ** (1 / (np.log(i + 3) + int((i + 3) == 1)) ** 2)) * alpha
